...
module.exports = class Checkpoint{
  constructor(id,position,type,radius,color,direction,dimension,visible,range,enter,leave){
  ...
  static config(file){
    ...
    for (let id in checkpoints){
      ...
      for (let i=0; i < checkpoint.positions.length; i++){
        new yarp.Checkpoint(id+" "+(i+1),checkpoint.positions[i],checkpoint.type,checkpoint.radius,checkpoint.color,checkpoint.direction,checkpoint.dimension,checkpoint.visible,checkpoint.range,checkpoint.enter,checkpoint.leave)
      }
    }
  }
